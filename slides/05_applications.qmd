...
```{r, engine='tikz', engine.opts=list(extra.preamble="\\usepackage{pgfplots}\\pgfplotsset{compat=1.18}"), fig.align='center', clean=TRUE}
...
1. Agents consume in both periods.
2. Production and government activity only in period $2$.
3. At $t=1$, agents receive an exogenous endowment $y$ every period, that is used to consume and invest: $$c_1+k_2=y$$

## Dynamic Inconsistency: Fiscal Policy

4. In the second period, individuals consume $c_2$, and government provides public goods with cost $g_2$.
5. There is a linear production function $$y_2= y+R k_2$$, with $R$ exogenous.
6. The budget constraint for the economy in $t=2$ is: $$c_2+g_2=Rk_2$$
...
We can collapse the budget constraint as: $$c_2+g_2=y+R(y-c_1)$$ and we get the Lagrangian:
<div style="font-size: 90%;">
...
\begin{align*}
    \frac{1}{c_1}&=\lambda R\\
    \frac{\beta}{c_2}&=\lambda\\
    \frac{\beta\gamma}{g_2}&=\lambda\\
    c_2+g_2&=y+R(y-c_1)
\end{align*}
</div>
...
<div style="font-size: 70%;">
\begin{align*}
    c_1&=\frac{y(1+R)}{R(1+\beta(1+\gamma))}\\
    c_2&=\beta R c_1\\
    g_2&=\gamma\beta R c_1\\
    k_2&= \frac{R\beta(1+\gamma)-1}{R(1+\beta(1+\gamma))}y
\end{align*}
</div>
...
\begin{align*}
\left.\begin{array}{c}
    \frac{1}{c_1}=\lambda(1-\tau)R\\
    \frac{\beta}{c_2}=\lambda
\end{array}\right\} \Rightarrow c_2=c_1\beta(1-\tau)R
\end{align*}
...
\begin{align}
    c_1\beta(1-\tau)R &= y + (1-\tau)R(y-c_1)\\
    c_1(1-\tau)R (1+\beta)&= \left(1 + (1-\tau) R\right)y\\
    c_1 &= \frac{1 + (1-\tau) R}{(1-\tau)R (1+\beta)}y
\end{align}
...
\begin{align*}
    k_2 &= y-c_1\\
    &= y-\frac{1 + (1-\tau) R}{(1-\tau)R (1+\beta)}y\\
    &= \frac{(1-\tau)R (1+\beta) - 1 - (1-\tau) R}{(1-\tau)R (1+\beta)}y\\
    &= \frac{(1-\tau)R \beta - 1 }{(1-\tau)R (1+\beta)}y\\
    k_2& = \frac{\beta}{1+\beta}y - \frac{1}{(1-\tau)R(1+\beta)}y
\end{align*}
...
\begin{align*}
    \frac{\gamma R k_2}{\tau R k_2} = \frac{Rk_2}{y+R(1-\tau)k_2}
\end{align*}

## Dynamic Inconsistency: Fiscal Policy

\begin{align*}
    \frac{\gamma}{\tau}&=\frac{Rk_2}{y+R(1-\tau)k_2}\\
    \tau&= \frac{\gamma(y+R(1-\tau)k_2)}{R k_2}\\
    \tau&= \frac{\gamma y}{R k_2}+\gamma(1-\tau)\\
    \tau &=\frac{(y+R k_2)}{R k_2}\frac{\gamma}{1+\gamma}
\end{align*}
...
\begin{align*}
    \max_{t_k}\ &\ln(y+R(1-\tau)k_2)+\gamma \ln(\tau R k_2)\\
    \max_{t_k}\ &\ln(y+R(1-\tau)k_2)+\gamma \ln (\tau)+\underbrace{\gamma\ln(R k_2)}_{\text{constant in}\ \tau}
\end{align*}
...
\begin{align*}
    \frac{\gamma}{\tau}&=\frac{Rk_2}{y+R(1-\tau)k_2}\\
    \tau Rk_2 &= \gamma y + \gamma R (1-\tau)k_2\\
    (1+\gamma)\tau R k_2 &= \gamma (y+Rk_2)\\
    \tau &= (y+Rk_2)\frac{\gamma}{1+\gamma}
\end{align*}
...
1. A Primer on Auction Design, Management and Strategy, David Salant.
2. Rules, Discretion and Reputation in a model of Monetary Policy, Robert Barro and David Gordon, 1983
3. Dynamic Inconsistency, Cooperation and the Benevolent Dissembling Government, Stanley Fisher, 1980
